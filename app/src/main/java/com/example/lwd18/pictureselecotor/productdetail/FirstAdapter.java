...
public class FirstAdapter extends RecyclerView.Adapter<FirstAdapter.MyViewHolder> {
  private List<TextSearchSameEntity.DataBean.ItemsBean> list;
  private int mI;
  ...
  @Override public void onBindViewHolder(final MyViewHolder holder, final int position) {
    ...
    TextSearchSameEntity.DataBean.ItemsBean ware = list.get(position);
    if (ware.getAmountType() == 1) {
      holder.llAmount.setVisibility(View.VISIBLE);
      holder.dpyhj.setVisibility(View.GONE);
      holder.amountPrice.setText(ware.getAmountMoney() + "");
    }else if (ware.getAmountType()==2){
      holder.llAmount.setVisibility(View.GONE);
      holder.dpyhj.setVisibility(View.VISIBLE);
      //holder.dpyhjtv.setText(ware.getAmountMoney()+"");
      holder.dpyhjtv.setText("满100减50");
    }
  ...
  public void addMoreItem(List<TextSearchSameEntity.DataBean.ItemsBean> newDatas) {
    list.addAll(newDatas);
    notifyDataSetChanged();
  }

  public void addItem(List<TextSearchSameEntity.DataBean.ItemsBean> mlists) {
    list.clear();
    mlists.addAll(list);
    list.removeAll(list);
    list.addAll(mlists);
