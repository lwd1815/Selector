...
import com.example.lwd18.pictureselecotor.event.SearchEvent;
import com.example.lwd18.pictureselecotor.event.SearchHistoryEvent;
import com.example.lwd18.pictureselecotor.textsearch.ProductDetailActivity;
import com.example.lwd18.pictureselecotor.textsearch.view.ConfirmDialog;
...
public class DuTextSearchActivity extends BaseDeepActivity {
  ...
  @BindView(R.id.et_search) com.deepbaytech.deeplibrary.OneKeyClearEditText etSearch;
  @BindView(R.id.tv_search_back) TextView tvSearchBack;
  @BindView(R.id.search_toolbar) android.support.v7.widget.Toolbar searchToolbar;
  ...
  private void initFlowLayout() {
    OkHttpUtils.get().url(ApiConstants.POPULAR_TEXT).build().execute(new StringCallback() {
   ...
   * 向FlowLayout中添加数据
   */
  private void addText(FlowLayout flowLayout, String text) {
    ...
    tvChild.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View v) {
        ...
        //goTextSearchResultActivity(flowKey);
      }
    });
  ...
  private void saveSearchHistory(String query) {
    ...
    list.add("牛奶");
    list.addAll(CacheManager.getInstance().getSearchHistory());
